...
use scylla_models::{Task, TaskStatus, UpdateOperation, TaskError, UpdateTaskModel};
...
fn validate_status_failure_scenarios() {
  ...
  assert_eq!(
    ...
    Err(ScyllaOperationsError::InvalidStatusTransition(TaskStatus::Running, vec![TaskStatus::Completed, TaskStatus::Cancelled, TaskStatus::Aborted]))
  );
...
fn validate_yield_operation_cases() {
  ...
  assert_eq!(validate_yield_operation(&t_ready), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Yield,
    TaskStatus::Running,
    TaskStatus::Ready,
  )));
  assert_eq!(validate_yield_operation(&t_aborted), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Yield,
    TaskStatus::Running,
    TaskStatus::Aborted,
  )));
  assert_eq!(validate_yield_operation(&t_cancelled), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Yield,
    TaskStatus::Running,
    TaskStatus::Cancelled,
  )));
  assert_eq!(validate_yield_operation(&t_completed), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Yield,
    TaskStatus::Running,
    TaskStatus::Completed,
  )));
...
fn validate_heart_beat_operation_cases() {
  ...
  assert_eq!(validate_heart_beat_operation(&t_ready), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::HeartBeat,
    TaskStatus::Running,
    TaskStatus::Ready,
  )));
  assert_eq!(validate_heart_beat_operation(&t_aborted), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::HeartBeat,
    TaskStatus::Running,
    TaskStatus::Aborted,
  )));
  assert_eq!(validate_heart_beat_operation(&t_cancelled), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::HeartBeat,
    TaskStatus::Running,
    TaskStatus::Cancelled,
  )));
  assert_eq!(validate_heart_beat_operation(&t_completed), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::HeartBeat,
    TaskStatus::Running,
    TaskStatus::Completed,
  )));
...
fn prepare_heart_beat_task_cases() {
  ...
  assert_eq!(prepared_task.deadline.unwrap() - Duration::seconds(10) - Utc::now() < Duration::milliseconds(1), true);
...
fn validate_lease_operation_cases() {
  ...
  assert_eq!(validate_lease_operation(&t_running, &utm_without_worker), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Lease,
    TaskStatus::Ready,
    TaskStatus::Running,
  )));
  assert_eq!(validate_lease_operation(&t_completed, &utm_without_worker), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Lease,
    TaskStatus::Ready,
    TaskStatus::Completed,
  )));
  assert_eq!(validate_lease_operation(&t_cancelled, &utm_without_worker), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Lease,
    TaskStatus::Ready,
    TaskStatus::Cancelled,
  )));
  assert_eq!(validate_lease_operation(&t_aborted, &utm_without_worker), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Lease,
    TaskStatus::Ready,
    TaskStatus::Aborted,
  )));
  assert_eq!(validate_lease_operation(&t_ready, &utm_without_worker),     Err(ScyllaOperationsError::MandatoryFieldMissing("worker".to_string(), UpdateOperation::Lease)));
...
fn prepare_lease_task_cases() {
  ...
  assert_eq!(prepared_task.deadline.unwrap() - Utc::now() - Duration::seconds(10) < Duration::milliseconds(1), true);
...
fn validate_reset_operation_cases() {
  ...
  assert_eq!(validate_reset_operation(&t_ready), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Reset,
    TaskStatus::Running,
    TaskStatus::Ready,
  )));
  assert_eq!(validate_reset_operation(&t_cancelled), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Reset,
    TaskStatus::Running,
    TaskStatus::Cancelled,
  )));
  assert_eq!(validate_reset_operation(&t_completed), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Reset,
    TaskStatus::Running,
    TaskStatus::Completed,
  )));
  assert_eq!(validate_reset_operation(&t_aborted), Err(ScyllaOperationsError::InvalidOperation(
    UpdateOperation::Reset,
    TaskStatus::Running,
    TaskStatus::Aborted,
  )));
/*********************************/
  ...
  assert_eq!(validate_reset_operation(&t_running_no_deadline), Err(ScyllaOperationsError::MandatoryFieldMissing("deadline".to_string(), UpdateOperation::Reset)));
  
  //  Running with valid deadline with still some time left

  assert_eq!(validate_reset_operation(&t_running_future_deadline), Err(ScyllaOperationsError::ValidationFailed("deadline not yet passed for reset operation".to_string())));
...
fn request_handler_cases() {
  ...
  assert_eq!(Utc::now() + Duration::seconds(10) - updated_task.deadline.unwrap() < Duration::milliseconds(1), true);
 /*********************************/
  // Status operation
  let t = Task {
    status: TaskStatus::Ready,
