...
use crate::task::{AddTaskModel, ScyllaOperationsError, ScyllaOperations};
...
fn update_task_returns_scylla_op_error() {
   ...
   assert_eq!(ScyllaOperations::update_task_operation(&utm, task_to_update), Err(ScyllaOperationsError::InvalidStatusTransition(TaskStatus::Ready, vec![TaskStatus::Cancelled])));
}
