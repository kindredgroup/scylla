...
async fn pg_manager_mock_adapter() {
   ...
   .on_insert(|t| Ok(t))
   .on_query_by_rn(|_rn| Ok(Task {
    rn: "query_by_rn".to_string(),
    ..Task::default()
   }))
   .on_query(|_gtm| Ok(vec![Task {
    rn: "query".to_string(),
    ..Task::default()
   }]))
   .on_update(|_t| Ok(Task {
    rn: "update".to_string(),
    ..Task::default()
   }));
    let pgm = PgManager {
        pg_adapter : Box::new(mock)
    };
   assert_eq!(pgm.fetch_task("rn".to_string()).await.unwrap().rn, "query_by_rn".to_string());
   assert_eq!(pgm.fetch_tasks(GetTaskModel { limit: None, queue: None, worker: None, status: None }).await.unwrap()[0].rn, "query".to_string());
   assert_eq!(pgm.insert_task(AddTaskModel { rn: "add".to_string(), spec: serde_json::Value::default(), priority: 1, queue: "s".to_string() }).await.unwrap().rn, "add".to_string());
   // update cases
   ...
   //heartbeat
  let mock = MockPgAdapter::default().on_query_by_rn(|_rn| Ok(Task {
    rn: "query_by_rn".to_string(),
   ...
   assert_eq!(pgm.abort_task("2".to_string(), TaskError { code: "123".to_string(), args: serde_json::Value::default(), description: "sd".to_string() }).await.unwrap().rn, "update".to_string());
