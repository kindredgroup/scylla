...
mod tests {
    ...
    fn scylla_operations_error_print_checks() {
        assert_eq!(
            PgAdapterError::ScyllaOpsError(ScyllaOperationsError::ValidationFailed("Field validation failed".to_string())).to_string(),
            "Validation failed: Field validation failed".to_string()
        );
        assert_eq!(
            PgAdapterError::DuplicateTask("sample".to_string()).to_string(),
            "Task already exist for sample".to_string()
        );
        assert_eq!(
            PgAdapterError::NoTaskFound("sample".to_string()).to_string(),
            "No task found for sample".to_string()
        );
        assert_eq!(
            format!("{:?}", PgAdapterError::DuplicateTask("sample".to_string())),
            "DuplicateTask(\"sample\")".to_string()
        );
    }
}
